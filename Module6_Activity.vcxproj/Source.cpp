...
void displayMultTable() {
	...
	// create a var to hold the value of the number the user wants to create a multiplication table for
	int userInput;
	...
	try {
		...
		if (userInput < 0) {
			throw runtime_error("No negative numbers allowed");
		}
...
// DISCLAIMER: All commentary in doubleAValue() is the same as in MultiplicationTable function, up to the callIntFunc. Skip ahead to see below:
void doubleAValue() {
	...
	try {
		...
		// Here, we call the callIntFunc again, but this time we specify a different function to call in the Python module
